...
always @(posedge emuclk) if(!phi1ncen_n) begin
    ...
    cyc1r_phase_prev <= ~(i_PG_PHASE_RST | i_TEST[2]) ? cyc18r_phase_sr_out : 19'd0;   // FIX: era ~i_PG_PHASE_RST (invertia el reset de fase -> sin sonido). Ahora igual que la rama pipelined (linea 134)
end
